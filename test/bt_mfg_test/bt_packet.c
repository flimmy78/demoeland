...
OSStatus bt_packet_pool_init( bt_packet_pool_t* pool, uint32_t packet_count, uint32_t header_size, uint32_t data_size )
{
    ...
    return mico_rtos_init_mutex( &pool->mutex );
}

OSStatus bt_packet_pool_deinit( bt_packet_pool_t* pool )
{
    uint32_t a;
    ...
    mico_rtos_deinit_mutex( &pool->mutex );
    ...
    for ( a = 0; a < packet_count; a++ )
    {
        ...
        if ( linked_list_remove_node_from_rear( &pool->pool_list, &removed_node ) == MICO_BT_SUCCESS )
        {
...
OSStatus bt_packet_pool_allocate_packet( bt_packet_pool_t* pool, bt_packet_t** packet )
{
    ...
    mico_rtos_lock_mutex( &pool->mutex );
    ...
    else if ( result == kNotFoundErr )
    {
        result = MICO_BT_PACKET_POOL_EXHAUSTED;
    }

    mico_rtos_unlock_mutex( &pool->mutex );

    return result;
}

OSStatus bt_packet_pool_dynamic_allocate_packet( bt_packet_t** packet, uint32_t header_size, uint32_t data_size )
{
    if ( packet == NULL )
    {
        return MICO_BT_BADARG;
...
OSStatus bt_packet_pool_free_packet( bt_packet_t* packet )
{
    if ( packet == NULL )
    {
        return MICO_BT_BADARG;
    ...
    else if ( packet->packet_id == PACKET_ID )
    {
        ...
        mico_rtos_lock_mutex( &packet->pool->mutex );
        ...
        mico_rtos_unlock_mutex( &packet->pool->mutex );
